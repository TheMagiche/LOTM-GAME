...
- [ ] GitHub Actions builds on push to `main` with `VITE_DEPLOYMENT_MODE=demo` ([`deploy.yml`](../../.github/workflows/deploy.yml))
- [ ] Coolify compose file is **`docker-compose.prod.yml`** (pull-only demo; `DEMO_MODE=1` / `TTS_DISABLED=1`)
...
| [`docker-compose.prod.yml`](../../docker-compose.prod.yml) | Coolify compose (pull-only Player demo) |
| [`docker-compose.demo.yml`](../../docker-compose.demo.yml) | Local demo build-arg overlay |
