...
## 3. Player-Only Enforcement (Code Backlog)

Today `uiViewMode` defaults to `'gm'` in [`settingsHelpers.ts`](../../src/store/slices/settingsHelpers.ts) and users can toggle in the Settings header ([`SettingsModal.tsx`](../../src/components/SettingsModal.tsx)). **Player mode is not enforced server-side.**

See [ui-view-modes.md](./ui-view-modes.md) for the existing Player mode contract:

- **Nav:** Play + Settings only (World, Engine, Mods hidden)
- **Settings tabs:** Providers, Presets, Advanced only

### Required greenfield work

| Task | Touch points |
|---|---|
| Build-time demo flag | `VITE_DEPLOYMENT_MODE=demo` in Dockerfile build args / GitHub Actions |
| Lock view mode | [`settingsHelpers.ts`](../../src/store/slices/settingsHelpers.ts) — default `player`; ignore `uiViewMode: 'gm'` updates when demo flag set |
| Hide GM toggle | [`SettingsModal.tsx`](../../src/components/SettingsModal.tsx) — omit `viewModeHeader` when demo |
| Optional: hide director injectors | [`ContextNavigationDrawer.tsx`](../../src/components/ContextNavigationDrawer.tsx) — Inject Arc/Event remain visible in Player mode today; product decision |
| BYOK onboarding modal | New `DemoOnboardingModal.tsx`; open when no provider has `apiKey` or `endpoint`; deep-link to Settings Providers |
| Server default settings | Optional `DEMO_MODE=1` env → seed [`data/settings.json`](../../server/lib/fileStore.js) with `uiViewMode: 'player'`, `ttsEnabled: false`, `aiTier: 'lite'` on first boot |
| Disable TTS on demo | Skip `warmupTts()` in [`server.js`](../../server.js) when `DEMO_MODE=1`; hide TTS controls in [`LotmDialoguePlate.tsx`](../../src/components/lotm/LotmDialoguePlate.tsx) / [`TtsPlaybackPanel.tsx`](../../src/components/tts/TtsPlaybackPanel.tsx) |
| Delete chronicle on exit | Hook [`exitLotmCampaign()`](../../src/components/lotm/LotmPlayHeader.tsx) → `deleteCampaign(id)` after final state flush; server already removes all campaign files + embeddings via [`DELETE /api/campaigns/:id`](../../server/routes/campaigns.js) |
| Session idle timeout | New demo session manager: issue opaque `demoSessionId` (cookie or sessionStorage); idle timer (e.g. 30–60 min) → logout UI + purge all campaigns tagged with that session |
| Demo world pack | Trimmed compendium build — see §4.3 |
...
- **Build-time:** `VITE_DEMO_COMPENDIUM=1` switches imports in `lordOfTheMysteries.ts` to `mechanics/World_compendium/Demo/` (new directory, not in repo today).
...
| `VITE_DEPLOYMENT_MODE` | `demo` (build-time — not wired today) |
...
- Do not expose GM view on public demo without auth.
...
- Do not ship the full 22-PC + full gamedata compendium if a trimmed demo build is available.
...
- Implementing landing page, routing, demo flags, ephemeral delete hooks, session timeout, demo compendium build, Electron CI, or auth (documented as backlog above)
