...
| **Lore RAG** | `src/services/lore/` | `loreChunker.ts`, `loreRetriever.ts`, `loreNPCParser.ts`, `loreFactionParser.ts`, `loreLocationParser.ts`, `loreEngineSeeder.ts`, `loreCheck.ts`, `loreKeywordEnricher.ts`, `lootTreeLoader.ts`, `worldLoreAI.ts`, `worldLoreExport.ts`, `worldLoreImport.ts` | Lore chunking with RAG hints, IDF+RRF retrieval (one winner per `##` group), lore-consistency verifier, world-builder AI. LOTM author sources live in `mechanics/.../lore/` and are concatenated in `src/worldpacks/lordOfTheMysteries.ts` before `chunkLoreFile()`. |
