...
import {
  ...
  EMPTY_REGISTER,
  getEntriesForNpc,
} from "../../services/campaign-state/divergenceRegister";
...
const INVENTORY_TABS: Array<{
  id: InventoryItemCategory | "all" | "equipped";
  label: string;
}> = [
  { id: "all", label: "All" },
...
export function LotmPlayerGrimoire() {
  ...
  const characterProfileData = useAppStore(
    (s) => s.characterProfileData ?? s.context.characterProfileData,
  );
  const inventoryItems = useAppStore(
    (s) => s.inventoryItems ?? s.context.inventoryItems ?? [],
  );
  ...
  const hudModel = useMemo(
    () =>
      buildLotmPlayerHudModel(playerCharacter, characterProfileData, {
  ...
  const characterName =
    playerCharacter?.name || characterProfileData?.name || "Player";
    ...
    >
      ...
      >
        <header className="lotm-grimoire-header">
          <div className="lotm-grimoire-brand">
            ...
            <div className="lotm-grimoire-titles">
              <p className="lotm-grimoire-kicker">
                Lord of the Mysteries · Player Grimoire
              </p>
              <h2 id="lotm-player-grimoire-title">
                {characterName}{" "}
                {hudModel.sequenceLabel ? `(${hudModel.sequenceLabel})` : ""}
              </h2>
            </div>
          </div>
        ...
        <div className="lotm-grimoire-body">
          <nav
            className="lotm-grimoire-rail"
            aria-label="Player Grimoire sections"
          >
          ...
          <div className="lotm-grimoire-main">
            ...
            {section === "inventory" && (
              <InventoryPane inventory={inventoryItems} />
            )}
            {section === "chronicle" && (
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-[#e0c36a] mt-0.5">
              {hudModel.sequenceLabel || "Uninitiated"}{" "}
              {pathway?.tarotCard ? `· ${pathway.tarotCard}` : ""}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2 min-w-55">
          <div>
            <div className="flex justify-between text-[11px] mb-1">
              <span className="font-['Cinzel'] text-[#c9a227] tracking-wider uppercase text-[9px]">
                Digestion
              </span>
          ...
          <div>
            <div className="flex justify-between text-[11px] mb-1">
              ...
              <span
                className={`text-[11px] ${locStage > 0 ? "text-red-400 font-semibold" : "text-emerald-300"}`}
              >
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 md:grid-cols-10 gap-2">
          {SEQUENCE_NUMBERS.map((num) => {
            ...
            const isDigested =
              currentSeqNum !== undefined && num > currentSeqNum;
          ...
          <div className="bg-[#0e0c11] border border-[#c9a227]/14 p-5 rounded-xl shadow-[inset_3px_3px_8px_#060507,inset_-3px_-3px_8px_#191620] space-y-3">
            <div className="flex items-center justify-between border-b border-[#c9a227]/20 pb-2">
              ...
              <span className="font-['Cinzel'] text-[9px] uppercase tracking-wider px-2 py-0.5 border border-[#c9a227]/30 text-[#e0c36a] rounded">
                {activeSeqNum === currentSeqNum
                  ? "Active Sequence"
                  : `Sequence ${activeSeqNum}`}
              </span>
            </div>
          ...
          <div className="bg-[#0e0c11] border border-[#c9a227]/14 p-5 rounded-xl shadow-[inset_3px_3px_8px_#060507,inset_-3px_-3px_8px_#191620] space-y-3">
            ...
            {abilities.length === 0 ? (
              <p className="text-xs italic text-[#a09075]">
                No explicit abilities recorded for this sequence.
              </p>
            ) : (
              <div className="space-y-2.5">
                {abilities.map((abilityName, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className="p-3.5 bg-[#141118] border border-[#c9a227]/14 rounded-lg shadow-[3px_3px_8px_#050407,-2px_-2px_6px_#1b1722] space-y-1"
                    >
                      <div className="flex items-center justify-between">
...
function LocationTravelPane({
  ...
  const [selectedPlaceIdOrName, setSelectedPlaceIdOrName] = useState<
    string | null
  ...
  const mapLocations = useMemo(() => {
    ...
    const existingNames = new Set(
      locationLedger.map((l) => l.name.toLowerCase()),
    );
  ...
  const activeLocationEntry = useMemo(() => {
    ...
    if (byId) return byId;
    const byName = locationLedger.find(
      (l) => l.name.toLowerCase() === selectedPlaceIdOrName.toLowerCase(),
    );
  ...
  const activeMapPin = useMemo(() => {
    ...
    return (
      INITIAL_LOCATIONS.find(
        (pin) => pin.name.toLowerCase() === selectedPlaceIdOrName.toLowerCase(),
      ) || null
    );
  ...
  const handleShiftLocation = (
    placeName: string,
    existingEntry?: LocationEntry | null,
  ) => {
    if (existingEntry) {
      updateContext({
    ...
    const match = locationLedger.find(
      (l) => l.name.toLowerCase() === placeName.toLowerCase(),
    );
    if (match) {
      updateContext({
    ...
    const pin = INITIAL_LOCATIONS.find(
    ...
    const newEntry: LocationEntry = {
      ...
      description:
        pin?.description ||
        `A notable location in the Lord of the Mysteries world.`,
      source: "manual",
  ...
  const isCurrentActive =
    activeLocationEntry && currentPlace?.id === activeLocationEntry.id;
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-[#d8cfbe]">
              {currentPlace?.broadLocation ||
                "The Northern & Southern Continents"}
      ...
      <div className="space-y-2">
        ...
        <div className="h-80 sm:h-96 min-h-85 border border-[#c9a227]/20 rounded-xl overflow-hidden bg-[#09080c] shadow-[inset_3px_3px_10px_#040306,inset_-3px_-3px_10px_#141118]">
          <LotmWorldMapView
            readOnly
            highlightCoords={
              activeLocationEntry?.coordinates || activeMapPin?.coordinates
            }
            onSelectName={(name) => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-3">
          ...
          <div className="relative">
            <Search
              size={14}
              className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#a09075]"
            />
            <input
              type="text"
          ...
          <div className="max-h-90 overflow-y-auto space-y-1.5 pr-1">
            ...
            {mapLocations.map((pin) => {
              const isSelected =
                !activeLocationEntry && activeMapPin?.name === pin.name;
              return (
                <button
                  key={pin.id}
                  type="button"
        ...
        <div className="lg:col-span-2">
          {activeLocationEntry || activeMapPin ? (
            <div className="bg-[#120f16] border border-[#c9a227]/30 p-5 rounded-sm space-y-4">
              <div className="flex items-center justify-between border-b border-[#c9a227]/20 pb-3">
                <div>
                  ...
                  <p className="text-xs text-[#c9a227]">
                    {activeLocationEntry?.broadLocation ||
              ...
              {activeLocationEntry?.features &&
                activeLocationEntry.features.length > 0 && (
                  <div>
                    <p className="font-['Cinzel'] text-[9px] uppercase tracking-wider text-[#c9a227] mb-2">
                      Features & Specific Locales (
                      {activeLocationEntry.features.length})
                    </p>
                    <div className="flex flex-wrap gap-2">
                      {activeLocationEntry.features.map((feat, idx) => {
                        const isSelectedFeature =
                          currentFeature === feat && isCurrentActive;
                        return (
                          <button
                            key={idx}
                            type="button"
            ...
            <div className="bg-[#0e0c11] border border-[#c9a227]/14 p-8 rounded-xl text-center text-sm text-[#a09075] shadow-[inset_3px_3px_8px_#060507,inset_-3px_-3px_8px_#191620]">
              Select a location from the map or list to view its details and
              travel.
            </div>
          )}
        </div>
...
function resolveCharacterRecordPortrait(
  pc: PlayerCharacter | null | undefined,
): string {
...
function CharacterGMPane({
  ...
  const traitTierMap = useMemo(
    () => Object.fromEntries(TRAIT_VOCAB.map((t) => [t.text, t.tier])),
    [],
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#121015] border border-[#c9a227]/16 p-5 rounded-xl shadow-[6px_6px_16px_#050408,-5px_-5px_14px_#1c1822] space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2.5 text-xs text-[#d8cfbe]">
            <div className="p-2.5 bg-[#0e0c11] rounded-lg border border-[#c9a227]/14 shadow-[inset_2px_2px_5px_#060507]">
              <span className="font-['Cinzel'] text-[8px] uppercase text-[#c9a227] block">
                Gender & Age
              </span>
              <span>
                {visual?.gender || "—"} · {visual?.age || "—"}
              </span>
            </div>
            <div className="p-2.5 bg-[#0e0c11] rounded-lg border border-[#c9a227]/14 shadow-[inset_2px_2px_5px_#060507]">
              <span className="font-['Cinzel'] text-[8px] uppercase text-[#c9a227] block">
            ...
            <div className="col-span-2 p-2.5 bg-[#0e0c11] rounded-lg border border-[#c9a227]/14 shadow-[inset_2px_2px_5px_#060507]">
              ...
              <span>
                {visual?.clothing ||
                  "Victorian formal frock coat, top hat, cane."}
              </span>
            </div>
        ...
        <div className="bg-[#121015] border border-[#c9a227]/16 p-5 rounded-xl shadow-[6px_6px_16px_#050408,-5px_-5px_14px_#1c1822] space-y-3">
          ...
          <div className="space-y-2 text-xs text-[#d8cfbe]">
            <div>
              <span className="font-['Cinzel'] text-[8px] uppercase tracking-wider text-[#c9a227] block mb-1">
                Disposition
              </span>
              <p className="bg-[#0e0c11] p-3 rounded-lg border border-[#c9a227]/14 shadow-[inset_2px_2px_6px_#060507,inset_-2px_-2px_6px_#191620] leading-relaxed">
                {pc?.personality ||
                  profile?.personality ||
                  "Pragmatic, cautious, attentive to mysticism."}
              </p>
            </div>
            <div>
              <span className="font-['Cinzel'] text-[8px] uppercase tracking-wider text-[#c9a227] block mb-1">
          ...
          <div className="grid grid-cols-2 gap-3">
            {HEX_AXES.map((axis) => {
              ...
              return (
                <div
                  key={axis}
                  className="p-2.5 bg-[#0e0c11] rounded-lg border border-[#c9a227]/14 shadow-[inset_2px_2px_6px_#060507,inset_-2px_-2px_6px_#191620]"
                >
                  <div className="flex justify-between items-center text-[10px] mb-1.5">
                    <span className="font-['Cinzel'] uppercase tracking-wider text-[#c9a227]">
                      {axis}
                    </span>
                    <span className="font-mono text-[#f3ead8]">
        ...
        <div className="bg-[#121015] border border-[#c9a227]/16 p-5 rounded-xl shadow-[6px_6px_16px_#050408,-5px_-5px_14px_#1c1822] space-y-3">
          ...
          <div className="space-y-3 text-xs">
            <div>
              <span className="font-['Cinzel'] text-[8px] uppercase tracking-wider text-[#c9a227] block mb-1.5">
                Core Traits
              </span>
              <div className="flex flex-wrap gap-2">
                {traits.length > 0 ? (
                  ...
                  <span className="text-xs italic text-[#8a7e6b]">
                    No core traits selected.
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="space-y-1.5 pt-1">
              <span className="font-['Cinzel'] text-[8px] uppercase tracking-wider text-[#c9a227] block">
                Wants & Aspirations
              </span>
              {wants?.short?.length ? (
                <p className="text-[11px] text-[#d8cfbe] bg-[#0e0c11] p-2.5 rounded-lg border border-[#c9a227]/12 shadow-[inset_2px_2px_5px_#060507]">
                  <span className="text-amber-400 font-semibold font-['Cinzel'] text-[9px] uppercase">
                    Short:{" "}
                  </span>
                  {wants.short.join("; ")}
                </p>
        ...
        <div className="bg-[#121015] border border-[#c9a227]/16 p-5 rounded-xl shadow-[6px_6px_16px_#050408,-5px_-5px_14px_#1c1822] space-y-3">
          ...
          <div className="space-y-3 text-xs text-[#d8cfbe]">
            {boundaries?.hard?.length ? (
              <div>
                <span className="font-['Cinzel'] text-[8px] uppercase tracking-wider text-red-400 block mb-1">
                  Hard Boundaries
                </span>
            ...
            {boundaries?.soft?.length ? (
              <div>
                <span className="font-['Cinzel'] text-[8px] uppercase tracking-wider text-amber-400 block mb-1">
                  Soft Boundaries
                </span>
              ...
              <div>
                ...
                <div className="space-y-1.5">
                  {triggers.map((trig, idx) => (
                    <div
                      key={idx}
                      className="p-2.5 bg-[#0e0c11] rounded-lg border border-[#c9a227]/14 shadow-[inset_2px_2px_5px_#060507] text-[11px]"
                    >
                      <span className="text-amber-300 font-semibold">
                        When:{" "}
                      </span>
                      <span>{trig.trigger}</span>
                      <span className="text-[#c9a227] font-semibold ml-2">
                        →{" "}
                      </span>
        ...
        <div className="bg-[#121015] border border-[#c9a227]/16 p-5 rounded-xl shadow-[6px_6px_16px_#050408,-5px_-5px_14px_#1c1822] space-y-3">
          ...
          <div className="space-y-2 text-xs text-[#d8cfbe]">
            {pc?.lore || profile?.backstory ? (
              ...
              <p className="italic text-[#8a7e6b]">
                No detailed story backstory logged.
              </p>
            )}
            <div className="grid grid-cols-2 gap-2 pt-1 text-[11px]">
              ...
              <div className="p-2.5 bg-[#0e0c11] rounded-lg border border-[#c9a227]/12 shadow-[inset_2px_2px_5px_#060507]">
                ...
                <span className="text-[#f3ead8]">
                  {pc?.origin || profile?.origin || "Loen Kingdom"}
                </span>
              </div>
            </div>
          </div>
...
function InventoryPane({ inventory }: { inventory: InventoryItem[] }) {
  const [selectedTab, setSelectedTab] = useState<
    InventoryItemCategory | "all" | "equipped"
  >("all");
  const [searchQuery, setSearchQuery] = useState("");
                ...
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {badge && (
                        <span
                          className={`text-[8px] font-['Cinzel'] uppercase px-2 py-0.5 rounded-md border shadow-[1px_1px_3px_#050407] ${
                            isSealedArtefact
...
function ChronicleStandingPane({
  ...
  const activeTraits = (profile?.activeTraits ?? []).filter(
    (t) => !t.superseded,
  );
  const supersededTraits = (profile?.activeTraits ?? []).filter(
    (t) => t.superseded,
  );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#121015] border border-[#c9a227]/20 p-5 rounded-xl shadow-[6px_6px_16px_#050408,-5px_-5px_14px_#1c1822] space-y-3">
        ...
        {activeTraits.length === 0 ? (
          <p className="text-xs italic text-[#8a7e6b]">
            No dynamic chronicle traits established yet.
          </p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {activeTraits.map((trait) => (
              <div
                key={trait.id}
                className="p-3.5 bg-[#0e0c11] border border-[#c9a227]/14 rounded-lg shadow-[inset_2px_2px_6px_#060507,inset_-2px_-2px_6px_#191620] space-y-1"
              >
                <div className="flex items-center justify-between">
                  <span className="font-['Cinzel'] text-[9px] uppercase tracking-wider text-[#c9a227]">
          ...
          <div className="pt-3 border-t border-[#c9a227]/15">
            ...
            <div className="space-y-1.5 opacity-60">
              {supersededTraits.map((trait) => (
                <p
                  key={trait.id}
                  className="text-xs text-[#a09075] line-through"
                >
                  {trait.text}
                </p>
              ))}
      ...
      <div className="bg-[#121015] border border-[#c9a227]/20 p-5 rounded-xl shadow-[6px_6px_16px_#050408,-5px_-5px_14px_#1c1822] space-y-3">
        ...
        {bonds.length === 0 ? (
          <p className="text-xs italic text-[#8a7e6b]">
            No distinct NPC standings established yet.
          </p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {bonds.map((npc) => {
              ...
              return (
                <div
                  key={npc.id}
                  className="p-3.5 bg-[#0e0c11] border border-[#c9a227]/14 rounded-lg shadow-[inset_2px_2px_6px_#060507,inset_-2px_-2px_6px_#191620] space-y-2"
                >
                  <div className="flex items-center justify-between">
                    <div>
                  ...
                  <div>
                    <div className="w-full h-2 bg-[#09080c] border border-[#c9a227]/15 rounded-full shadow-[inset_1px_1px_3px_#040305,inset_-1px_-1px_3px_#141118] overflow-hidden mb-1">
                      <div
                        ...
                        style={{
                          width: `${Math.round(((clamped + 3) / 6) * 100)}%`,
                        }}
                      />
                    </div>
      ...
      <div className="bg-[#121015] border border-[#c9a227]/20 p-5 rounded-xl shadow-[6px_6px_16px_#050408,-5px_-5px_14px_#1c1822] space-y-3">
        ...
        {divergenceEntries.length === 0 ? (
          <p className="text-xs italic text-[#8a7e6b]">
            No divergence chronicle events logged for the protagonist.
          </p>
        ) : (
          <div className="space-y-2.5">
            {divergenceEntries.map((entry) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span
                      className={`font-['Cinzel'] text-[9px] uppercase tracking-wider ${color}`}
                    >
